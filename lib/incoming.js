...
Incoming.prototype._SYMBOL_SAMPLES = function () {
  var id = this.dequeueInt();
  ...
  while (itemCount--) {
    ...
    var nDerivativeSecTypes = this.dequeueInt();
    contract.derivativeSecTypes = [];
    for (var i = 0;i<nDerivativeSecTypes;i++) {
      derivSecType = this.dequeue();
      contract.derivativeSecTypes.push(derivSecType);
    }

    contracts.push(contract);
  }

  this._emit('symbolSamples', id, contracts);
};
