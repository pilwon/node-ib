...
Incoming.prototype._TICK_BY_TICK = function () {
    ...
    switch (tickType) {
        ...
        case 2: // Alllast
            ...
            this._emit('tickByTickAllLast', reqId, tickType, time, price, size, {
                pastLimit,
                unreported
...
Incoming.prototype._NEWS_PROVIDERS = function () {
    const providers = [];
    try {
        let i;
    ...
    this._emit('reqNewsProviders', providers);
};
Incoming.prototype._HISTORICAL_NEWS = function () {
    ...
    for (let i = 0; i < 300; i++) {
        const headline = {};
        this.dequeue();
        headline.date = this.dequeue();
        if (headline.date === '0') {
            break;
        }
        this.dequeue();
        this.dequeue();
        headline.title = this.dequeue();
        this.dequeue();

        providers.push(headline);
    }
    this._emit('reqHistoricalNews', providers);
};
