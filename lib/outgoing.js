...
Outgoing.prototype.reqMatchingSymbols = function (reqId, pattern) {
  if (this._controller._serverVersion < C.MIN_SERVER_VER.REQ_MATCHING_SYMBOLS) {
    return this._controller.emitError('It does not support stock contract search.');
  }

  this._send(C.OUTGOING.REQ_MATCHING_SYMBOLS, reqId, pattern);
};
