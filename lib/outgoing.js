...
Outgoing.prototype.placeOrder = function (id, contract, order) {
  ...
  if (this._controller._serverVersion < C.MIN_SERVER_VER.ORDER_COMBO_LEGS_PRICE && BAG_SEC_TYPE.equalsIgnoreCase(contract.secType)) {
  ...
  if (this._controller._serverVersion < C.MIN_SERVER_VER.ORDER_COMBO_LEGS_PRICE) {
    ...
    args.push(order.lmtPrice || Number.MAX_VALUE);
  }
  if (this._controller._serverVersion < C.MIN_SERVER_VER.TRAILING_PERCENT) {
    args.push(order.auxPrice === Number.MAX_VALUE ? 0 : order.auxPrice);
  } else {
    args.push(order.auxPrice || Number.MAX_VALUE);
  }
  ...
  if (this._controller._serverVersion >= 8 && BAG_SEC_TYPE.equalsIgnoreCase(contract.secType)) {
    if (!_.isArray(contract.comboLegs)) {
      args.push(0);
    } else {
  ...
  if (this._controller._serverVersion >= C.MIN_SERVER_VER.ORDER_COMBO_LEGS_PRICE &&
    ...
    if (!_.isArray(order.orderComboLegs)) {
      ...
      order.orderComboLegs.forEach(function (orderComboLeg) {
        args.push(orderComboLeg.price || Number.MAX_VALUE);
      });
    }
  }
  ...
  if (this._controller._serverVersion >= C.MIN_SERVER_VER.SMART_COMBO_ROUTING_PARAMS &&
      BAG_SEC_TYPE.equalsIgnoreCase(contract.secType)) {
  ...
  if (this._controller._serverVersion >= 19) {
    ...
    args.push(order.minQty || Number.MAX_VALUE);
    args.push(order.percentOffset || Number.MAX_VALUE);
    args.push(order.eTradeOnly);
    args.push(order.firmQuoteOnly);
    args.push(order.nbboPriceCap || Number.MAX_VALUE);
    args.push(order.auctionStrategy || Number.MAX_VALUE);
    args.push(order.startingPrice || Number.MAX_VALUE);
    args.push(order.stockRefPrice || Number.MAX_VALUE);
    args.push(order.delta || Number.MAX_VALUE);
    ...
    args.push(lower || Number.MAX_VALUE);
    args.push(upper || Number.MAX_VALUE);
  }
  ...
  if (this._controller._serverVersion >= 26) { // Volatility orders
     args.push(order.volatility);
     args.push(order.volatilityType);
    ...
    if (this._controller._serverVersion < 28) {
      ...
      args.push(order.deltaNeutralAuxPrice || Number.MAX_VALUE);
    ...
    args.push(order.referencePriceType || Number.MAX_VALUE);
  }

  if (this._controller._serverVersion >= 30) { // TRAIL_STOP_LIMIT stop price
    args.push(order.trailStopPrice || Number.MAX_VALUE);
  }

  if (this._controller._serverVersion >= C.MIN_SERVER_VER.TRAILING_PERCENT){
    args.push(order.trailingPercent || Number.MAX_VALUE);
  }

  if (this._controller._serverVersion >= C.MIN_SERVER_VER.SCALE_ORDERS) {
    if (this._controller._serverVersion >= C.MIN_SERVER_VER.SCALE_ORDERS2) {
     args.push(order.scaleInitLevelSize || Number.MAX_VALUE);
     args.push(order.scaleSubsLevelSize || Number.MAX_VALUE);
    } else {
      args.push('');
      args.push(order.scaleInitLevelSize || Number.MAX_VALUE);
    }
    args.push(order.scalePriceIncrement|| Number.MAX_VALUE);
  }

  if (this._controller._serverVersion >= C.MIN_SERVER_VER.SCALE_ORDERS3 &&
      order.scalePriceIncrement > 0.0 &&
      order.scalePriceIncrement != Number.MAX_VALUE) {
    args.push(order.scalePriceAdjustValue || Number.MAX_VALUE);
    args.push(order.scalePriceAdjustInterval || Number.MAX_VALUE);
    args.push(order.scaleProfitOffset || Number.MAX_VALUE);
    args.push(order.scaleAutoReset);
    args.push(order.scaleInitPosition || Number.MAX_VALUE);
    args.push(order.scaleInitFillQty || Number.MAX_VALUE);
